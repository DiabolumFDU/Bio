...
#define DEVICE_NAME_DEFAULT "/dev/xdma0_c2h_1"
#define SIZE_DEFAULT (0x80)
...
static int test_dma(char *devicename, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filename)
{
  ...
  posix_memalign((void **)&allocated, 4096/*alignment*/, size * 20);
  assert(allocated);
  ...
  while (count--) {
	...
	i++;
	//printf("%d package received!\n", i);
		//printf("First frame of the current package\n");
	//	for(m=0;m<32;m++){
	//		printf("0x%x ",(char) buffer[m]);
	//		if (buffer[m] == 0x0a) {
	//			printf("buffer[%d] = 0x0a\n", m);
	if(buffer[23] == 0x0a){
		flag_break=1;
		printf("End of Package found!\n");
	}
	//	printf("\n");
		//printf("continue?\n");
		//getchar();
  ...
  printf("CLOCK_MONOTONIC reports %ld.%09ld seconds (total) for %d transfers of 128 bytes\n", ts_end.tv_sec, ts_end.tv_nsec, i);
